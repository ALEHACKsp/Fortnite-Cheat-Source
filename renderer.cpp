...
namespace rend {
	...
	VOID EndScene(ImGuiWindow& window)
	{
		...
		ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0.4f, 0.4f, 0.4f, 0.4f));
		ImGui::SetNextWindowSize(ImVec2(379, 400), ImGuiSetCond_FirstUseEver);

		static int tab = 1;

		if (showMenu)
		{
			if (ImGui::Begin(xorstr("Exotic V2 (RAGE & LEGIT"), 0, ImGuiWindowFlags_::ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_::ImGuiWindowFlags_NoResize))
			{
				ImGui::Separator();

				if (ImGui::Button("Discord"))
				{
					system("start https://discord.gg/zfGM4cd6Jq");
				}

				ImGui::Separator();

				ImGui::Text("Owner VITAL#9494");
				...
				ImGui::SliderFloat(xorstr("Smooth##slider"), &Settings.AimbotSlow, 0.00f, 20.00f, xorstr("%.2f"));
				ImGui::SliderFloat(xorstr("Aim Fov##slider"), &Settings.AimbotFOV, 80.0f, 150.0f, xorstr("%.2f"));
	...
	VOID AddLine(ImGuiWindow& window, float width, float height, float a[3], float b[3], ImU32 color, float& minX, float& maxX, float& minY, float& maxY) {
		...
		if (Util::WorldToScreen(width, height, ac) && Util::WorldToScreen(width, height, bc)) {
			window.DrawList->AddLine(ImVec2(ac[0], ac[1]), ImVec2(bc[0], bc[1]), color, 0.1f);
